...
[カメラ / ギャラリー画像]
        │
        ▼
① 画像前処理（ImagePreprocessor / RecognizeActivity）
  　長辺を 1280px にリサイズ（EXIF 回転補正はギャラリー読み込み時に実施）
        │
        ▼
...
| 処理 | 担当 | 内容 |
|---|---|---|
| **リサイズ** | `ImagePreprocessor.kt` | 長辺を最大 1280px に統一。短辺は縦横比を維持してスケール |
| **EXIF 回転補正** | `RecognizeActivity.kt` | ギャラリーから画像を読み込む際に EXIF タグを読み取り、正しい向きに回転してから認識に渡す |

> **なぜ前処理はリサイズのみか**：実機テストの結果、二値化・グレースケール変換・コントラスト強調などの外部前処理は PaddleOCR の認識精度を**低下させる**ことが確認されました。PaddleOCR は内部で正規化を行うため、カラー画像をそのまま渡す方が高精度です。

> **EXIF について**：スマートフォンのカメラは「保存時に向きを回転させず EXIF に記録する」仕様のため、EXIF を無視すると 90° 回転した状態で認識されてしまいます。本アプリではギャラリー読み込み時に自動補正しています。
